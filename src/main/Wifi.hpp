...
void setupWifi() {
  ...
  WiFiProvisioner::Config customCfg(
      "Alarm Configuration",   // Access Point Name
      "Welcome to WiFi configuration", // HTML Page Title
      "#0989d8",                     // Theme Color
      ...
      "Alarm config", // Project Title
      "Setup the wifi system",       // Project Sub-title
      "Connection to WiFi to access weather and other updates",
                                     // Project Information
      "Current page displayed thanks to © Custom WiFiProvisioner"
      "System created by eGuardian Development. https://eguardian.dev", // Footer
                                                      // Text
      "The device is now connected to WiFi.", // Success Message
  ...
  provisioner.onProvision([]() { Serial.println("Provisioning started."); })
      ...
      .onSuccess([](const char *ssid, const char *password, const char *input) {
        ...
        SettingsEEPROM.begin("wifiSettings", RW_MODE);

        logger.debug(String("Connected to SSID: ") + ssid);
        SettingsEEPROM.putString("SSID_Name",String(ssid));
        if (password) {
          SettingsEEPROM.putString("SSID_Pass",String(password));
          logger.debug(String("Password: ") + password);
        }else{
          SettingsEEPROM.putString("SSID_Pass",String(""));
        }
        ...
        logger.log("Provisioning completed successfully!");
      })
      .onFactoryReset([]() { logger.warning("Factory reset triggered!"); 
      });

  // Start provisioning

  SettingsEEPROM.begin("wifiSettings", RO_MODE);  

  bool wifiAccount = SettingsEEPROM.isKey("SSID_Name");
  ...
  if(!wifiAccount){
    ...
    String wifiName = SettingsEEPROM.getString("SSID_Name");
    String wifiPass = SettingsEEPROM.getString("SSID_Pass");
    SettingsEEPROM.end();
    logger.debug(String("wifi is: ")+ wifiName);
    logger.debug(String("wifi password is: ")+ wifiPass);
